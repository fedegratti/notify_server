...
RSpec.describe Notification, type: :model do
  ...
  describe "validations" do
    ...
    context "title validation" do
      it "is invalid without a title" do
        notification = Notification.new(valid_attributes.except(:title))
      ...
      it "is invalid with a blank title" do
        notification = Notification.new(valid_attributes.merge(title: ""))
    ...
    context "content validation" do
      it "is invalid without content" do
        notification = Notification.new(valid_attributes.except(:content))
      ...
      it "is invalid with blank content" do
        notification = Notification.new(valid_attributes.merge(content: ""))
    ...
    context "channel validation" do
      it "is valid with a valid channel" do
        notification = Notification.new(valid_attributes.merge(channel: Notification.channels.keys.sample))
  ...
  describe "associations" do
    ...
    it "is invalid without a user" do
      notification = Notification.new(valid_attributes.except(:user))
      expect(notification).not_to be_valid
  ...
  describe "enums" do
    it "defines channel enum" do
      expect(Notification.channels).to eq({
        "email" => 0,
        "sms" => 1,
        "push" => 2
      })
  ...
  describe "scopes and queries" do
    ...
    it "can filter by channel using enum scopes" do
      expect(Notification.email.count).to eq(2)
      expect(Notification.sms.count).to eq(3)
      expect(Notification.push.count).to eq(1)
    end
  end
end
